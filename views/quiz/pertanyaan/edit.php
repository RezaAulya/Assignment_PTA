...
<?php // PARSING 
	...
	$sql = "SELECT quiz_soal, quizid FROM quizinfo WHERE infoid = '$get_id' AND userid = '$userid'";
    $result = mysqli_query($db, $sql);
    $row = mysqli_fetch_array($result);
	$nama_header = $row['quiz_soal'];
	
	
    $form_name ="Edit Pertanyaan - ".$nama_header;
    $quizid = $row['quizid'];
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		$sql = "UPDATE quizinfo SET quiz_soal = '$quiz_soal', ";
		
		$temp = 5-$totalPilihan;
		$ins = "";
		for ($i = 0; $i < $totalPilihan; $i++){
			$val = mysqli_real_escape_string($db, $pilihanJawaban[$i]);
			$ins .= "quiz_jawaban_".($i+1)." = '$val', ";
		}
		
		for ($i = $temp; $i < 5; $i++){
			$ins.= "quiz_jawaban_".($i+1)." = NULL, ";
		}
		$sql .= $ins."quiz_jawaban = '$quiz_jawaban' WHERE infoid = '$get_id' AND userid = '$userid';";
        echo $sql;
  
		$result = mysqli_query($db, $sql);
		header("Location: ../pertanyaan-view/".$quizid);
		exit;
	}
?>
                            ...
                            <div class="col-sm-12">
                                <form class="form-horizontal" role="form" method="post" id="question_bank" enctype="multipart/form-data">
                                    ...
                                    <div class='form-group'>
                                        ...
                                        <div class="col-sm-6">
                                            <textarea class="form-control" id="question" name="question"><?php echo $nama_header;?></textarea>
                                        </div>
                                        <span class="col-sm-4 control-label">
                                        </span>
                                    </div>
